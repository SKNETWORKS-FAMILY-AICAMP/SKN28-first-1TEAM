...
def render(dp):
    st.title("시간대별 지역 데이터 지도")

    # 1. 지도 데이터 로드
    # script_dir = os.path.dirname(os.path.realpath(__file__))
    # file_path = os.path.join(script_dir, 'seoul_geo_gu.json')
    ...
    if view_option == "사고량":
        ...
        col, label, scale = 'traffic', '교통량', "Viridis"

    time_slots = sorted(map_data['time_slot'].unique())
    selected_time = st.select_slider("시간대 선택", options=time_slots)
    ...
    fig = px.choropleth_mapbox(
        ...
        mapbox_style="carto-positron", zoom=10, center={"lat": 37.5665, "lon": 126.9780},
        opacity=0.7, labels={col: label}
    )
    ...
    st.caption(f"현재 선택된 데이터: {view_option} | 시간대: {selected_time}")