...
def render(dp):
    ...
    st.info("사고량, 교통량 및 등록 차량 대비 포화도를 실시간으로 분석합니다.")

    # 1. 지도 데이터(GeoJSON) 및 구별 좌표 로드
    with open("app/seoul_geo_gu.json", encoding='utf-8') as f:
        seoul_geo = json.load(f)
    ...
    # 2. 지표 선택 (라디오 버튼)
    view_option = st.radio("표시할 지표를 선택하세요", ["사고량", "교통량", "교통 포화도"], horizontal=True)
    ...
    # 6. 하단 요약 정보 (선택 사항)
    st.caption(f"💡 {selected_time} 기준 {view_option} 분석 결과입니다. (스케일: {min_range:,.1f} ~ {max_range:,.1f})")