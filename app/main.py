...
# 분리된 페이지들을 임포트
from src import summary, accident_map, traffic, danger, faq
...
menu = st.sidebar.selectbox("메뉴 선택", ["전체 요약", "사고 지도 히트맵", "종합 지수 분석", "위험 지수 분석", "FAQ"])

# 페이지 라우팅 (선택된 메뉴에 따라 해당 파일의 render 함수 실행)
if menu == "전체 요약":
    summary.render(dp)
elif menu == "사고 지도 히트맵":
    accident_map.render(dp, seoul_geo)
